    ...
    <div class="card">
        <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-4 col-md-2 text-center">
                                <div class="text-truncate mb-1">
                                    <div class="bg-{{ ($thread->creator->online()) ? 'success' : 'danger' }} mr-1" style="height:9px;width:9px;border-radius:50px;display:inline-block;"></div>
                            ...
                            <div class="col-8 col-md-10">
                                ...
                                @if (Auth::check() && Auth::user()->isStaff())
                                    ...
                                    @if (Auth::user()->staff('can_delete_forum_posts'))
                                        <a href="{{ route('forum.moderate', ['thread', 'delete', $thread->id]) }}" class="btn btn-{{ (!$thread->is_deleted) ? 'gray' : 'danger' }}"><i class="fas fa-trash" style="width:13px;"></i></a>
                                    @endif

                                    @if (Auth::user()->staff('can_edit_forum_posts'))
                                        <a href="{{ route('forum.edit', ['thread', $thread->id]) }}" class="btn btn-gray"><i class="fas fa-edit" style="width:13px;"></i></a>
                                    @endif

                                    @if (Auth::user()->staff('can_pin_forum_posts'))
                                        <a href="{{ route('forum.moderate', ['thread', 'pin', $thread->id]) }}" class="btn btn-{{ (!$thread->is_pinned) ? 'gray' : 'danger' }}"><i class="fas fa-thumbtack" style="width:13px;"></i></a>
                                    @endif

                                    @if (Auth::user()->staff('can_lock_forum_posts'))
                                        <a href="{{ route('forum.moderate', ['thread', 'lock', $thread->id]) }}" class="btn btn-{{ (!$thread->is_locked) ? 'gray' : 'danger' }}"><i class="fas fa-lock" style="width:13px;"></i></a>
                                    @endif
                                @endif
                            </div>
                        </div>
            ...
            @foreach ($thread->replies() as $reply)
                <div class="card" style="@if ($reply->is_deleted)opacity:0.5;@endif">
                    <div class="card-body" style="padding:15px 0;">
                        @auth
                        ...
                        <div class="row">
                            <div class="col-4 col-md-2 text-center">
                                <div class="text-truncate mb-1">
                                    <div class="bg-{{ ($reply->creator->online()) ? 'success' : 'danger' }} mr-1" style="height:9px;width:9px;border-radius:50px;display:inline-block;"></div>
                            ...
                            <div class="col-8 col-md-10">
                                ...
                                @if (Auth::check() && Auth::user()->isStaff())
                                    ...
                                    @if (Auth::user()->staff('can_delete_forum_posts'))
                                        <a href="{{ route('forum.moderate', ['reply', 'delete', $reply->id]) }}" class="btn btn-{{ (!$reply->is_deleted) ? 'gray' : 'danger' }}"><i class="fas fa-trash" style="width:13px;"></i></a>
                                    @endif

                                    @if (Auth::user()->staff('can_edit_forum_posts'))
                                        <a href="{{ route('forum.edit', ['reply', $reply->id]) }}" class="btn btn-gray"><i class="fas fa-edit" style="width:13px;"></i></a>
                                    @endif
                                @endif
                            </div>
                        </div>
