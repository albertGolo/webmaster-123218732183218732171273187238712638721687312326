...
function getMembers(rank, page)
{
    $.get('/api/groups/members', { id, rank, page }).done((data) => {
        ...
        $.each(data.members, function() {
            $('#members').append(`<div class="col-2 text-center">
    <a href="${this.url}"><img src="${this.thumbnail}" class="img-fluid"></a>
...
function getWallPosts(page)
{
    $.get('/api/groups/wall', { id, page }).done((data) => {
        ...
        $.each(data.posts, function() {
            ...
            $('#wall').append(`<div class="card">
  <div class="card-body">
      <div class="row">
          <div class="col-auto">
              <img src="${this.creator.thumbnail}" style="height: 100px; width: 100px;" class="img-fluid">
          </div>
          <div class="col ml-n2">
              <h4 class="mb-1">
                  <a href="${this.creator.url}">${this.creator.username}</a>
              </h4>
              <div>${this.body}</div>
              <div style="height: 20px;"></div>
              <div class="text-muted"><i class="fas fa-clock"></i> ${this.time_ago}</div>
          </div>
          <div class="col-auto">
              <div class="dropdown">
                  <a href="#_" class="dropdown-ellipses dropdown-toggle" role="button" data-toggle="dropdown" aria-haspopup="true" data-expanded="false">
