...
class RegisterController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        $validate = [
			...
			'displayname' => ['required', 'min:3', 'max:20', 'regex:/\\A[a-z\\d]+(?:[.-][a-z\\d]+)*\\z/i', 'unique:users'],
        ...
        $this->validate($request, $validate, [
			...
			'displayname.unique' => 'Display Name has already been taken.',
        ...
        if (config('app.env') == 'production' && $accountsWithSameIP >= 1)
            return back()->withErrors(['You are only allowed to make one account at this time.']);
