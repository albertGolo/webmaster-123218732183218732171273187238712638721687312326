...
class CatalogController extends Controller
{
    public function index()
    {
        ...
        foreach (config('catalog_categories') as $category) {
            $itemArray = [];
            $columns = [];

            $columns[] = ['status', '=', 'approved'];
            $columns[] = ['public_view', '=', true];

            if (!isset($category['is_visible']) || !$category['is_visible'])
                continue;

            if (isset($category['official_only']) && $category['official_only'])
                $columns[] = ['creator_id', '=', 1];

            if (isset($category['limiteds_only']) && $category['limiteds_only'])
                $columns[] = ['limited', '=', true];

            if (isset($category['name_like']))
                $columns[] = ['name', 'LIKE', "%{$category['name_like']}%"];

            if (isset($category['item_ids']) && !empty($category['item_ids'])) {
                foreach ($category['item_ids'] as $item) {
                    $tmp = $columns;
                    $tmp[] = ['id', '=', $item];
            ...
            else if (isset($category['item_types']) && !empty($category['item_types']))
                $data->items = Item::where($columns)->whereIn('type', $category['item_types'])->take(50)->orderBy('updated_at', 'DESC')->get();
            else
                $data->items = Item::where($columns)->take(50)->orderBy('updated_at', 'DESC')->get();
    ...
    public function item($id, $slug)
    {
        ...
        if (!$item->public_view && (!Auth::check() || !Auth::user()->isStaff())) abort(404);
        ...
        return view('web.catalog.item')->with([
            'item' => $item,
            'suggestions' => Item::whereIn('id', $item->recommendations())->get(),
