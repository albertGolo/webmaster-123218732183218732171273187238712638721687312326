...
class User extends Authenticatable
{
    ...
    {
        return !empty($value ?? '') ? $value : $this->username;
    }
    ...
    public function thumbnail()
    {
        ...
        return Storage::url("thumbnails/{$image}.png");
    }
    ...
    public function headshot()
    {
        ...
        return Storage::url("thumbnails/{$image}_headshot.png");
    }
    ...
    {
        return 100 * pow(1.50, $this->forum_level - 1);
    }
    ...
    public function ownsItemInBundle($bundleId)
    {
        $bundleItems = BundleItem::where('bundle_id', '=', $bundleId)->get();

        foreach ($bundleItems as $item) {
            if ($this->ownsItem($item->item_id))
                return true;
        }
    ...
    public function grantBundleItems($bundleId)
    {
        $bundleItems = BundleItem::where('bundle_id', '=', $bundleId)->get();
    ...
    {
        return Cache::remember("user:{$this->id}:groups", 3600, function () {
    ...
    {
        return Cache::remember("user:{$this->id}:badges", 3600, function () {
    ...
    {
        return Cache::remember("user:{$this->id}:friend_requests", 3600, fn () => Friend::where([
            ['receiver_id', '=', $this->id],
            ['is_pending', '=', true]
        ])->count());
    }
    ...
    {
        return Cache::remember("user:{$this->id}:friends", 3600, fn () => $this->friendsOfMine()->wherePivot('is_pending', '=', false)->orderBy('created_at', 'DESC')->get()->merge($this->friendOf()->wherePivot('is_pending', '=', false)->orderBy('created_at', 'DESC')->get()));
    }
    ...
    {
        return Cache::remember("user:{$this->id}:forum_post_count", 600, fn () => ForumThread::where([
            ['creator_id', '=', $this->id],
            ['is_deleted', '=', false]
